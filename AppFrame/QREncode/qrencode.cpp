...
static unsigned char QRraw_getCode(QRRawCode *raw)
{
	...
	if(raw->count < raw->dataLength) {
		...
		{
			int total=0;
			for(int i=0;i<row;i++)	{total += raw->rsblock[i].dataLength;}
			g_QRBitData.JHbitsStreamLocation_dir = (total+col)*8;
		}
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		{
			int total=0;
			total+=raw->dataLength;
			for(int i=0;i<row;i++)	{total += raw->rsblock[i].eccLength;}
			g_QRBitData.JHbitsStreamLocation_dir = (total+col)*8;
		}
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	} else {
...
static unsigned char *FrameFiller_next(FrameFiller *filler)
{
	...
	if(filler->bit == -1) {
		...
		if(locateFlag)
		{
			moved=0;
			remainderMoved=0;
			if(g_QRBitData.JHbitsStreamLocation_dir!=-1) {
				g_QRBitData.JHbitsStreamLocation[g_QRBitData.JHbitsStreamLocation_dir+moved] = filler->y * filler->width + filler->x;
			}
			moved++;
			if(moved==8)moved=0;
		}
	...
	{
		if(g_QRBitData.JHbitsStreamLocation_dir!=-1) {
			g_QRBitData.JHbitsStreamLocation[g_QRBitData.JHbitsStreamLocation_dir+moved] = y * w + x;
			moved++;
		}
		else if(g_QRBitData.JHbitsStreamLocation_dir==-1) {
			g_QRBitData.JHbitsStreamLocation[g_QRBitData.JHbitsStreamTotalLength-g_QRBitData.JHbitsStreamRemaiderLength+remainderMoved]= y * w + x;
			remainderMoved++;
		}
		if(moved==8)moved=0;
	}
...
//	fillerC = FrameFiller_new(width, frame, 0);
	...
	if(locateFlag)
	{
		g_QRBitData.JHbitsStreamModeLength = 4;
		g_QRBitData.JHbitsStreamCounterLength = (input->version>=10)?16:8;
		g_QRBitData.JHbitsStreamDataLength = raw->dataLength*8;
		g_QRBitData.JHbitsStreamECCLength  = raw->eccLength*8;
		g_QRBitData.JHbitsStreamTerminatorLength = 4;
		g_QRBitData.JHbitsStreamRemaiderLength = QRspec_getRemainder(raw->version);

		g_QRBitData.JHbitsStreamTotalLength = 
			g_QRBitData.JHbitsStreamDataLength+
			g_QRBitData.JHbitsStreamECCLength+
			g_QRBitData.JHbitsStreamRemaiderLength;

		g_QRBitData.JHbitsStreamLocation.clear();
		g_QRBitData.JHbitsStreamLocation.resize(g_QRBitData.JHbitsStreamTotalLength);
		g_QRBitData.JHbitsStreamLocation_dir=-1;
	}
	////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//      printf("%d ",*p);
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if(locateFlag){g_QRBitData.JHbitsStreamLocation_dir=-1;}
	////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
{
	if (locateFlag){g_QRBitData.JHbitsStreamRealStringLength = (strlen(string) )* 8;}
