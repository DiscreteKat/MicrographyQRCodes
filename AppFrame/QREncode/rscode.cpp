...
//#define NULL ((void *)0)
  ...
  for (i = 0,root=fcr*prim; i < nroots; i++,root += prim) {
    ...
    for (j = i; j > 0; j--){
      if (rs->genpoly[j] != 0)	rs->genpoly[j] = rs->genpoly[j-1] ^ rs->alpha_to[modnn(rs,rs->index_of[rs->genpoly[j]] + root)];
...
void encode_rs_char(RS *rs, const data_t *data, data_t *parity)
{
  ...
  for(i=0;i<NN-NROOTS-PAD;i++){
    ...
    if(feedback != A0)    parity[NROOTS-1] = ALPHA_TO[MODNN(feedback + GENPOLY[0])];
    else			      parity[NROOTS-1] = 0;
  }
}
