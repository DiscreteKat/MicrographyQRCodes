...
{
	EnergyType new_energy, old_energy, ori_energy;
	...
	try 
	{
		if ( max_num_iterations == -1 )
		{
			...
			ori_energy = new_energy;
			old_energy = 1+new_energy;
			printf(" expansion...%lf      ",new_energy);
			do
			{
				gcoclock_t ticks0 = gcoclock();
				m_stepsThisCycle = 0; 
				...
				else // All expansions were successful, so do another complete sweep
				{
					next = 0;  

					//**HSS
					new_energy = compute_energy();
					if (new_energy>=old_energy){flag=false;}
					else{old_energy = new_energy;}
				}
				printStatus1(cycle++,false,ticks0);
			} while ( !queueSizes.empty() && flag);

			printf("\r expansion...%lf to %lf     \n",ori_energy,new_energy);
		}
		else
		{
...
{
	EnergyType new_energy,old_energy,init_energy;
	if ( (this->*m_solveSpecialCases)(new_energy) )
		return new_energy;
	
	init_energy = compute_energy();
	...
	try
	{
		printf(" swap...%d %lf           ",curr_cycle,new_energy);
		while ( old_energy > new_energy && curr_cycle <= max_num_iterations)
		{
			...
			curr_cycle++;
			printf("\rswap...%d %lf           ",curr_cycle,new_energy);
		}
		printf("\r swap%d...%lf to %lf           \n",curr_cycle,init_energy,new_energy);
	} 
	catch (...)
	{
		m_stepsThisCycle = m_stepsThisCycleTotal = 0;
